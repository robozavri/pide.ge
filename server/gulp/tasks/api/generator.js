...
function buildReference(key) {
  if (refFields[key].referenceType === 'multiple') {
    return `[{ type: Schema.Types.ObjectId, ref: '${_.capitalize(refFields[key].reference)}' }]`;
  }
  return `{ type: Schema.Types.ObjectId, ref: '${_.capitalize(refFields[key].reference)}' }`;
}
...
export function generateKeywordSearch(fields = false) {
  ...
  Object.keys(fields).map((key, index) => {
    switch( fields[key] ) {
      case 'multilingualSchema':
        template += buildKeyword(key, 'multilingualSchema');
        break;
      case 'String':
        template += buildKeyword(key, 'String');
        break;
    }
  });
  return template;
...
function buildKeyword(key, value) {
  let template = '';
  if (value == 'String') {
    return `{ '${key}': { $regex: keyword, $options: 'i' } },\n\u0020\u0020\u0020\u0020\u0020\u0020`;
  }

  if (value == 'multilingualSchema') {
    availableLangs.map((lang) => {
      template += `{ '${key}.${lang}': { $regex: keyword, $options: 'i' } },\n\u0020\u0020\u0020\u0020\u0020\u0020`;
    });
    return template;
  }
}
...
function generateMetaObj(key) {
  return  `{
    title : {
      ${buildMultilingualelement(`${key} title`)}
    },
    description : {
      ${buildMultilingualelement(`${key} description`)}
    },
